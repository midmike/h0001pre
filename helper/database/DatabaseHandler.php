...
class DatabaseHandler {
	...
	private static function GetHandler() {
		if (! isset ( self::$_mHandler )) {
			try {
				self::$_mHandler = new PDO ( PDO_DSN, DB_USERNAME, DB_PASSWORD, array (
	...
	public static function GetAllWithBinding($statementHandler, $bindings, $fetchStyle = PDO::FETCH_ASSOC) {
		$result = null;
		try {
			if (is_array ( $bindings )) {
				for($i = 0, $ien = count ( $bindings ); $i < $ien; $i ++) {
					$binding = $bindings [$i];
					$statementHandler->bindValue ( $binding ['key'], $binding ['val'], $binding ['type'] );
				}
			}
			self::Execute ( $statementHandler );
			$result = $statementHandler->fetchAll ( $fetchStyle );
		} catch ( PDOException $e ) {
			self::Close ();
			echo "Class DatabaseHandler.GetAll: " . $e->getMessage ();
